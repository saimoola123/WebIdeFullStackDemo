sap.ui.define(["sap/ui/core/mvc/Controller"], function (Controller) {
	"use strict";
	return Controller.extend("sai.moola.WEBIDEFSDEMO.controller.Fruits", {
		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf sai.moola.WEBIDEFSDEMO.view.Fruits
		 */
		onInit: function () {
		...
		handleNavigationWithContext: function () {
			...
			function _onRouteMatched(oEvent) {
				...
				// 	return key + "=" + (oProp.type === "Edm.String" ? "'" + oProp + "'" : oProp);
				// }).join(",");

				oView.bindElement({
					path: entitySet + "('" +JSON.parse(decodeURIComponent(oArgs["SuppliedId"])) + "')",
					events: {
